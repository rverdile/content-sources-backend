...
func ConfigureEchoWithMetrics(ctx context.Context, metrics *instrumentation.Metrics) *echo.Echo {
	...
	if config.Get().Clients.RbacEnabled {
		...
		if config.Get().Features.Kessel.Enabled {
			kesselURL := config.Get().Clients.Kessel.URL
			kesselTimeout := time.Duration(int64(config.Get().Clients.Kessel.Timeout) * int64(time.Second))
			rbacBaseUrl := config.Get().Clients.RbacBaseUrl
			authEnabled := config.Get().Clients.Kessel.AuthEnabled
			clientID := config.Get().Clients.Kessel.AuthClientID
			clientSecret := config.Get().Clients.Kessel.AuthClientSecret
			oidcIssuer := config.Get().Clients.Kessel.AuthOIDCIssuer
